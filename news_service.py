...
class NewsService():
    ...
    def get_most_error_dates(self):
        ...
        errors_query = '''
                SELECT to_char(requests.time, 'FMMonth DD, YYYY') AS time,
                CAST (errors.total * 100 / requests.total AS decimal(10,2))
                AS percentage
                FROM total_reqs requests
